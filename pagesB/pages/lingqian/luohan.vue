<template>
	<view class="page bg-white">
		...
		<u-popup v-model="showWin" v-if="Object.keys(list).length > 0" border-radius="20" mode="center" width="95%"
			:closeable="true">
			<view class="qianBox">
				<view class="qianTitle u-text-center">罗汉灵签第{{list.index}}签：{{list.zun_zhe}}</view>
				<view class="qianTitle l-b-t-primary">签语</view>
				<view class="qianContent">
					<u-parse :html="nl2br(list.qian_yu)"></u-parse>
				</view>
				<view class="qianTitle l-b-t-primary">偈解</view>
				<view class="qianContent">
					<u-parse :html="nl2br(list.ji_jie)"></u-parse>
				</view>
			</view>
		</u-popup>
	</view>
	...
	export default {
		...
		methods: {
			getChouQian() {
				...
				setTimeout(() => {
					//开始查询
					that.post('ling_qian/luo_han', {}, res => {
						that.disabled = false;
						that.showWin = true;
